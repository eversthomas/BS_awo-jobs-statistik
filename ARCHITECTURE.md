...
## Planung weiterer Verbesserungen - Feature: `Zusatzinformationen` (JSON) als Metafelder auswerten

**Ziel:** Das JSON-Feld `"Zusatzinformationen"` wird im Plugin als strukturierte Metadaten gelesen, um **(1) Fachbereich**, **(2) alte Stellennummer (Legacy)** und **(3) VZÄ/FTE** zuverlässig zu übernehmen – auch wenn einzelne Werte fehlen.

### Eingabeformat (Option B)
Immer alle Keys, Values dürfen leer sein:

`FB=<...> | LEGACY=<...> | FTE=<...>`

Beispiele:
- `FB=Kita | LEGACY=4711 | FTE=0.75`
- `FB=Kita | LEGACY= | FTE=0.75`
- `FB= | LEGACY= | FTE=`

### Parsing-Regeln
- Trennzeichen: `|`
- Key/Value: `=`
- **Key-basiert** parsen (niemals positionsbasiert), Keys case-insensitive (`fb` = `FB`)
- Erlaubte Keys: `FB`, `LEGACY`, `FTE`
- Unbekannte Segmente ignorieren (optional: Warning/Log)
- Mehrfache Keys: **last wins**
- Leere Values → `null`

### Normalisierung
- `FB`: String trim, leer → `null`
- `LEGACY`: String trim, leer → `null`
- `FTE`: `,` zu `.` normalisieren, dann numeric → float, sonst `null` (optional: Warning)

### Datenmodell / Nutzung im Plugin
- Pro Stelle die drei Werte in der Import-Pipeline verfügbar machen und speichern (Post-Meta/DB-Spalten/Struktur analog zum bestehenden Plugin).
- `LEGACY` wird für Jahreswechsel-Matching nutzbar: **Fortführung erkennen**, wenn neue Stelle `LEGACY=<alte Stellennr>` gesetzt hat.

### DoD (kurz)
- Import übernimmt `FB`, `LEGACY`, `FTE` korrekt aus `"Zusatzinformationen"`
- Fehlende Werte führen nicht zu falschen Zuordnungen
- Mindestens 3 Tests/Beispielfälle: vollständig, Legacy leer, FTE ungültig
